...
var HorseController_exports = {};
__export(HorseController_exports, {
  HorseController: () => HorseController
});
module.exports = __toCommonJS(HorseController_exports);
...
var HorseController = class {
  async uploadImage(req, res) {
    ...
    repo2.uploadImage({ req, res, controller });
  }
  async getHorse(req, res) {
    const { uuid: uuidhorse } = req.params;
    const result = await repo2.getHorse({ uuidhorse });
  ...
  async delete(req, res) {
    try {
      const { uuid: uuidhorse } = req.params;
      const result = await repo2.delete({ uuidhorse });
      if (result instanceof Error)
        return res.status(422).json({
          status: "01",
          message: result.message,
