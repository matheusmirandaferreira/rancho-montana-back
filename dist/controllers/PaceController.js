...
var PaceController_exports = {};
__export(PaceController_exports, {
  PaceController: () => PaceController
});
module.exports = __toCommonJS(PaceController_exports);
...
var repo = AppDataSource.getRepository(Pace);
var PaceRepository = class {
  async getPace({ uuidpace }) {
    if (!(0, import_uuid.validate)(uuidpace))
...
var PaceController = class {
  ...
  async create(req, res) {
    try {
      ...
      const result = await repo2.create({ nmpace });
      if (result instanceof Error)
  ...
  async edit(req, res) {
    try {
      ...
      const { nmpace } = req.body;
      const result = await repo2.edit({ uuid, nmpace });
      if (result instanceof Error)
        return res.status(422).json({
          status: "01",
          message: result.message,
