...
var repo = AppDataSource.getRepository(Color);
var ColorRepository = class {
  async getColor({ uuidcolor }) {
    if (!(0, import_uuid.validate)(uuidcolor))
  ...
  async create(props) {
    let { nmcolor } = props;
    nmcolor = (0, import_normalize_text.normalizeWhiteSpaces)(nmcolor);
    if (!nmcolor)
    ...
    const permalink = (0, import_normalize_text.normalizeDiacritics)(nmcolor).toLowerCase().replaceAll(" ", "_");
...
var ColorController = class {
  ...
  async create(req, res) {
    try {
      ...
      const result = await repo2.create({ nmcolor });
      if (result instanceof Error)
  ...
  async edit(req, res) {
    try {
      ...
      const { nmcolor } = req.body;
      const result = await repo2.edit({ uuid, nmcolor });
      if (result instanceof Error)
        return res.status(422).json({
          status: "01",
          message: result.message,
